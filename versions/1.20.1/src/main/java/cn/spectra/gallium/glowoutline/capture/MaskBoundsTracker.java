...
public final class MaskBoundsTracker {
    ...
    private static GlowCaptureState owner;
    ...
    public static void begin(GlowCaptureState state, int width, int height, boolean baseKnown) {
        ...
        current = state.maskBounds; owner=state;
    ...
    public static void end() { current = null; owner=null; }
    static boolean canUseBaseVertex() { return baseVertexSafe; }

    public static void record(RenderedBuffer mesh, RenderType type) {
        ...
        try {
            ...
            if (shader == null || positionOffset(format) < 0) {
                current.invalidate(); return;
            }
            ...
            cn.spectra.gallium.glowoutline.shader.OutlineTemporalStabilizer.mesh(owner,mesh,
                    RenderSystem.getModelViewMatrix(),RenderSystem.getProjectionMatrix(),program);
            current.include(mesh.vertexBuffer(), format.getVertexSize(), state.vertexCount(),
                    positionOffset(format),
    ...
    private static int positionOffset(VertexFormat format) {
        int offset = 0;
        for (var element : format.getElements()) {
            if (element.isPosition()) return element.getType() == VertexFormatElement.Type.FLOAT && element.getCount() == 3 ? offset : -1;
