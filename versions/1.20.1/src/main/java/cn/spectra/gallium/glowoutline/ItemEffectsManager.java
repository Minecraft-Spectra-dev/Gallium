...
public class ItemEffectsManager implements ResourceManagerReloadListener {
    ...
    public static final ResourceLocation RELOAD_ID = new ResourceLocation("gallium", "item_effects");
    private static final ResourceLocation RESOURCE_PATH = new ResourceLocation("gallium", "item_effects.json");
    ...
    private static ItemCondition parseConditionNodeInner(JsonObject node, int ruleIndex, String path) {
        ...
        if (!pathValue.startsWith("components.") && !pathValue.startsWith("nbt.")) {
            Gallium.LOGGER.warn("item_effects rule[{}] {}: invalid predicate path '{}'", ruleIndex, path, pathValue);
            return null;
        }
        String compType = LegacyItemData.parsePath(pathValue);
        if (compType == null) {
            Gallium.LOGGER.warn("item_effects rule[{}] {}: unsupported 1.20.1 path '{}'", ruleIndex, path, pathValue);
            return new ItemCondition.Or(List.of());
        }
