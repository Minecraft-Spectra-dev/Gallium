...
bool storedMask() {
#ifdef GALLIUM_HAS_MASK_STORAGE
    return GalliumMaskStorage.z == 1.0;
#else
    return false;
#endif
}
...
#ifdef GALLIUM_HAS_MASK_STORAGE
    if (!storedHand()) return texelFetch(GalliumMaskBaseDepthSampler,
                clamp(coord, ivec2(0), textureSize(GalliumMaskBaseDepthSampler, 0) - 1), 0).r;
#endif
    return 1.0;
}
...
// A fully covered center makes the original outline factor non-positive.
// Keep the original main/finalizeGlow path, including its alpha and color math.
bool gallium_CoveredCenter = false;
float isItem(vec2 uv) {
    float coverage = reconstructedMask(uv, 1.0, false);
    gallium_CoveredCenter = coverage >= 1.0;
    return coverage;
}

float destinationDepthAt(vec2 uv, out bool valid) {
    if (gallium_CoveredCenter) {
        valid = false;
        return 1.0;
    }
