...
//$$   valid &= seen;motion.begin();depthCache.begin();geometry.clear();nearestDepth.clear();OutlineMaskFootprints.beginFrame();
//#if MC==1_21_11
//$$   OutlineSrCapture.beginFrame();
//#endif
...
//#if MC==1_21_11
//$$    +OutlineSrCapture.reservedBytes()
//#endif
//$$    ;}
//#if MC==1_21_11
//$$  static boolean capturingSr(){return enabled;}
//#endif
...
//#else
//$$   return cn.spectra.gallium.glowoutline.IrisCompat.isActiveSrRuntime()
//$$       && !cn.spectra.gallium.glowoutline.SuperResolutionCompat.isHackConfigured();
//#endif
//$$  }
...
//#else
//$$   return cn.spectra.gallium.glowoutline.SuperResolutionCompat.hasCompletedShaderCompatDispatch(target.width,target.height);
//#endif
//$$  }
...
//$$      && destination.getColorTexture() instanceof GlTexture color && color.glId()==destinationTexture)copy(after,destinationTexture);
//$$    unavailable(failure);
...
//$$   before=after=width=height=index=0;Arrays.fill(colors,0);Arrays.fill(depths,0);reserved=0;valid=false;motion.clear();depthCache.clear();previous.clear();geometry.clear();nearestDepth.clear();OutlineMaskFootprints.release();
//#if MC==1_21_11
//$$   OutlineSrCapture.release();
//#endif
//$$  }
...
//$$   resolve=resolveCpu=resolveGpu=combine=vao=fbo=0;copied=enabled=seen=false;OutlineMaskFootprints.dispose();depthPrograms.clear();
//#if MC==1_21_11
//$$   OutlineSrCapture.dispose();OutlineNativeOcclusion.dispose();
//#endif
//$$  }
//$$  public static void dispose(){disposeObjects();disabled=false;jitterSeen=false;jitterLifetime=0;}
//$$ 
//$$  static void stored(NativeMaskAtlas.Entry entry){if(enabled)OutlineMaskFootprints.observe(entry);}
//$$  public static void mesh(GlowCaptureState state,MeshData mesh,Matrix4fc modelView,Matrix4fc projection,int program){
//$$   if(!enabled || state==null || state.firstPerson)return;
//$$   var d=mesh.drawState();var f=d.format();if(!f.contains(VertexFormatElement.POSITION))return;
//$$   var b=mesh.vertexBuffer();int stride=f.getVertexSize(),offset=f.getOffset(VertexFormatElement.POSITION);geometry.add(state);
...
//$$    if(OutlineMaskFootprints.gpuCapable())try{resolveGpu=program(RESOLVE.replace("#version 330 core", "#version 330 core\n#define GALLIUM_GPU_FOOTPRINTS 1"));}
//$$    catch(RuntimeException|LinkageError unavailable){OutlineMaskFootprints.disableGpu();cn.spectra.gallium.Gallium.LOGGER.warn("GPU footprint consumer unavailable; keeping the readback path",unavailable);}
//$$   }
...
//$$   var value=ReflectiveSrRuntimeAccess.getInstance().shaderValue(SourceKind.UNIFORM,"taaJitter");
...
//#if MC==1_21_11
//$$   bytes=OutlineHistoryPolicy.srStorageBytes(target.width,target.height);
//$$   var transform=cn.spectra.gallium.glowoutline.IrisCompat.getShaderProjectionTransform(target.width,target.height);
//$$   // Display masks must be unjittered. Nonzero input origins need their own mapping.
//$$   if(transform.viewportOriginX()!=0 || transform.viewportOriginY()!=0){releaseHistory();return;}
//#endif
//$$   if(!GlowCaptureManager.temporalHistoryFitsBudget(target.width,target.height,bytes)){releaseHistory();return;}
//$$   if(!(target.getColorTexture() instanceof GlTexture color) || color.getClass()!=GlTexture.class
//$$     || GL45.glGetTextureLevelParameteri(color.glId(),0,GL11.GL_TEXTURE_INTERNAL_FORMAT)!=GL11.GL_RGBA8){valid=false;return;}
//$$   ensure(target.width,target.height);destination=target;destinationTexture=color.glId();
...
//$$  private static Rect rect(GlowCaptureState s){
//$$   var footprint=OutlineMaskFootprints.get(s);
//$$   if(footprint!=null && footprint.rectangle()!=null){var r=footprint.rectangle();return new Rect(r.x(),r.y(),r.width(),r.height());}
//$$   return ownerRect(s);
//$$  }
//$$  private static Rect ownerRect(GlowCaptureState s){
//$$   var c=BoundedGlowContracts.get(s.config.shader());
//#if MC==1_21_11
//$$   var observed=OutlineSrCapture.bounds(s);
//#else
...
//$$     || !(target.getColorTexture() instanceof GlTexture color) || color.glId()!=destinationTexture){valid=false;return;}seen=true;
...
//$$   
//#if MC==1_21_11
//$$   if(!OutlineSrCapture.valid()){valid=false;return;}
//#endif
...
//#if MC==1_21_11
//$$   var nativeEntries=OutlineSrCapture.entries();
//$$   if(nativeEntries.size()!=states.size()){valid=false;return;}
//$$   // Rebuild only scratch RGB; keep the displayed raw frame intact on failure.
//$$   originalCaptured=false;
//$$   OutlineNativeOcclusion.render(before,after,depths[1-index],width,height,nativeEntries);
...
//$$   var forced=Collections.newSetFromMap(new IdentityHashMap<GlowCaptureState,Boolean>());
//$$   for(var s:states)if(!s.firstPerson){var old=previous.get(s);if(old==null || old.hand || motion.changed(s) || old.config!=s.config.hashCode())forced.add(s);}
//$$   OutlineMaskFootprints.force(forced);
//$$   if(!OutlineMaskFootprints.prepare(states,width,height)){valid=false;return;}
//$$   boolean gpuFootprints=OutlineMaskFootprints.gpuMode();
//$$   if(OutlineMaskFootprints.modeChanged()){valid=false;previous.clear();}
//$$   resolve=gpuFootprints?resolveGpu:resolveCpu;
//$$   for(var s:states){
...
//$$    if(gpuFootprints){if(old!=null && old.hand)rejectPrevious(reject,old);}
//$$    else if(old==null || motion.changed(s) || old.config!=s.config.hashCode() || (!moving && OutlineMaskFootprints.get(s).changed())){reject.add(r);rejectPrevious(reject,old);}
//$$    next.put(s,new Previous(r,gpuFootprints?OutlineMaskFootprints.get(s).slot():-1,false,s.config.hashCode()));
//$$   }
...
//$$   int scene=((GlTexture)GlowCaptureManager.getSceneDepthTarget().getDepthTexture()).glId();
//$$   int output=((GlTexture)target.getColorTexture()).glId();int write=1-index;
//$$   boolean srgb=GL11.glIsEnabled(GL30.GL_FRAMEBUFFER_SRGB),blend1=GL30.glIsEnabledi(GL11.GL_BLEND,1);
//$$   try(var saved=new OutlineTemporalGlState();var stack=MemoryStack.stackPush();var bindings=OutlineMaskFootprints.bind(resolve)){
...
//$$ }
//#else
public final class OutlineTemporalStabilizer { private OutlineTemporalStabilizer() {} }
//#endif
