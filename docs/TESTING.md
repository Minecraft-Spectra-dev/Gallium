...
- Fabric 1.20.1 使用 Java 17、Sodium `0.5.13+mc1.20.1` 与 Iris `1.7.6+mc1.20.1`；验证关闭光影及开启光影的场景。按用户要求，此版本暂不验收 SR 同装，未来 Forge 适配时再处理。
...
- [ ] 第一人称物品穿入墙体时描边仍完整；手臂、地图不得产生额外捕获，打开 GUI 不干扰手持描边。1.20.1 另用没有世界描边物体、仅主手或主副手持描边物品的场景检查实际窗口输出，HUD 暗角开启时不能黑屏；捕获/合成计数正常不能代替画面检查。
